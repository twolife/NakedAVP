...
extern void ClearScreenToBlack();
...
int BinkDecodeFrameInternal(struct binkMovie* aMovie, AVPacket* aPacket)
{
	...
	if(aPacket->stream_index == aMovie->videoStreamIndex)
	{
		...
		if(avcodec_send_packet(aMovie->videoCodecContext, aPacket) == 0 &&
		   avcodec_receive_frame(aMovie->videoCodecContext, aMovie->videoFrame) == 0)
			decoded_frame_ready = 1;
		else
			return aMovie->looping;

		if(decoded_frame_ready == 0)
			return 1;
	...
	else if(aPacket->stream_index == aMovie->audioStreamIndex)
	{
		...
		if(avcodec_receive_frame(aMovie->audioCodecContext, aMovie->audioFrame) < 0 &&
		   avcodec_send_packet(aMovie->audioCodecContext, aPacket) < 0)
			return aMovie->looping;

		if(!SoundSys_IsOn())
			return 0;

		if(!aMovie->alInited && aMovie->audioFrame->ch_layout.u.mask != 0)
		{
			...
			switch(aMovie->audioFrame->ch_layout.u.mask)
			{
...
int BinkStartMovie(struct binkMovie* aMovie, const char* aFilename, BOOL aLoopFlag, BOOL aFmvFlag, BOOL aMusicFlag)
{
	...
	char *fullpath = getFullPath(aFilename);
	int ret = avformat_open_input(&aMovie->avContext, fullpath, NULL, NULL);
	free(fullpath);
	if(ret < 0)
	{
		return 0;
	...
	for(int i=0; i<aMovie->avContext->nb_streams; i++)
	{
		...
		AVCodecParameters* codec_par = aMovie->avContext->streams[i]->codecpar;
		AVCodec const * codec = avcodec_find_decoder(codec_par->codec_id);
		if(codec)
		{
		        codec_context = avcodec_alloc_context3(codec);
		        avcodec_parameters_to_context(codec_context, codec_par);
			if((codec_par->codec_type==AVMEDIA_TYPE_VIDEO && aMovie->videoStreamIndex>=0) || (codec_par->codec_type==AVMEDIA_TYPE_AUDIO && aMovie->audioStreamIndex>=0))
				continue;

			if(avcodec_open2(codec_context, codec, 0) < 0)
				continue;

			if(codec_par->codec_type==AVMEDIA_TYPE_VIDEO && !aMusicFlag)
			{
...
void PlayBinkedFMV(char *filenamePtr)
{
	...
	{
		alSourcef(movie.alSource, AL_GAIN, PlatVolumeToGain(60));
